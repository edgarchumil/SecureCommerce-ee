...
import { addTreatment, createRisk, getRiskBands, getRisks, getThreats, getTreatments, getVulnerabilities } from '../api/risks'
import type { Risk, RiskBand, RiskLevel, RiskPayload } from '../types/risks'
...
export function RisksPage() {
  ...
  const risks = useQuery({ queryKey: ['risks', search, level, page], queryFn: () => getRisks(search, level, page) })
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-8"><section className="mx-auto max-w-6xl">
    ...
    {bands.data && <RiskMatrix bands={bands.data} />}
    {bands.isError && <p role="alert" className="mt-4 text-red-800">No fue posible cargar la escala de riesgo de la empresa.</p>}
    <div className="mt-7 flex flex-wrap gap-3"><label className="relative min-w-64 flex-1"><span className="sr-only">Buscar riesgos</span><Search className="absolute left-3 top-3 text-slate-500" size={18} /><input className="w-full rounded-lg border border-slate-300 bg-white py-2.5 pl-10 pr-3" placeholder="Buscar por código o título" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} /></label><select aria-label="Filtrar por nivel" className="rounded-lg border border-slate-300 bg-white px-3" value={level} onChange={(e) => { setLevel(e.target.value); setPage(1) }}><option value="">Todos los niveles</option>{Object.entries(LEVELS).map(([key, item]) => <option key={key} value={key}>{item.label}</option>)}</select></div>
    ...
    {risks.data && risks.data.total > 0 && <nav aria-label="Paginación de riesgos" className="mt-4 flex flex-wrap items-center justify-between gap-3"><p>{risks.data.total} riesgos · Página {risks.data.page} de {risks.data.pages}</p><div className="flex gap-3"><button className="rounded-lg border px-4 py-2 disabled:opacity-40" disabled={page <= 1 || risks.isFetching} onClick={() => setPage(page - 1)}>Anterior</button><button className="rounded-lg border px-4 py-2 disabled:opacity-40" disabled={page >= risks.data.pages || risks.isFetching} onClick={() => setPage(page + 1)}>Siguiente</button></div></nav>}
...
function RiskMatrix({ bands }: { bands: RiskBand[] }) { return <section className="mt-7 rounded-xl border bg-white p-5"><h2 className="font-semibold text-blue-950">Matriz 5 × 5</h2><p className="mb-3 text-sm text-slate-500">Impacto → / Probabilidad ↓</p><div className="grid max-w-md grid-cols-5 gap-1">{[5,4,3,2,1].flatMap((probability) => [1,2,3,4,5].map((impact) => { const score = probability * impact; const level = bands.find(band => band.minimum <= score && score <= band.maximum)?.level; return <div key={`${probability}-${impact}`} title={`Probabilidad ${probability}, impacto ${impact}${level ? ` · ${LEVELS[level].label}` : ''}`} className={`flex aspect-square items-center justify-center rounded font-bold ${level ? LEVELS[level].style : 'bg-slate-100'}`}>{score}</div> }))}</div><p className="mt-3 text-sm text-slate-600">{bands.map(band => `${LEVELS[band.level].label}: ${band.minimum}–${band.maximum}`).join(' · ')}</p></section> }
